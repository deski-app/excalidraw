...
import { getSnapLinesAtPointer, snapDraggedElements, isActiveToolNonLinearSnappable, snapNewElement, snapResizingElements, isSnappingEnabled, getVisibleGaps, getReferenceSnapPoints, SnapCache, } from "../snapping";
...
class App extends React.Component {
    ...
    renderEmbeddables() {
        ...
        return (_jsx(_Fragment, { children: embeddableElements.map((el) => {
                ...
                return (_jsx("div", { className: clsx("excalidraw__embeddable-container", {
                        ...
                        opacity: getRenderOpacity(el, getContainingFrame(el), this.elementsPendingErasure),
    ...
    OPENAI_KEY = EditorLocalStorage.get(EDITOR_LS_KEYS.OAI_API_KEY);
    ...
    onOpenAIKeyChange = (openAIKey, shouldPersist) => {
        ...
        if (shouldPersist) {
            const didPersist = EditorLocalStorage.set(EDITOR_LS_KEYS.OAI_API_KEY, openAIKey);
    ...
    handleLinearElementOnPointerDown = (event, elementType, pointerDownState) => {
        if (this.state.multiElement) {
            ...
            const [gridX, gridY] = getGridPoint(pointerDownState.origin.x, pointerDownState.origin.y, event[KEYS.CTRL_OR_CMD] ? null : this.state.gridSize);
            const topLayerFrame = this.getTopLayerFrameAtSceneCoords({
                x: gridX,
                y: gridY,
            });
        ...
        return withBatchedUpdatesThrottled((event) => {
            ...
            if (draggingElement.type === "freedraw") {
                ...
                if (shouldRotateWithDiscreteAngle(event) && points.length === 2) {
                    ({ width: dx, height: dy } = getLockedLinearCursorAlignSize(draggingElement.x, draggingElement.y, pointerCoords.x, pointerCoords.y));
                }
                if (points.length === 1) {
                    mutateElement(draggingElement, {
                        points: [...points, [dx, dy]],
    ...
    onPointerUpFromPointerDownHandler(pointerDownState) {
        return withBatchedUpdates((childEvent) => {
            ...
            const { draggingElement, resizingElement, multiElement, activeTool, isResizing, isRotating, } = this.state;
            this.setState((prevState) => ({
                isResizing: false,
                isRotating: false,
            ...
            if (!activeTool.locked &&
                ...
                this.setState((prevState) => ({
                    ...
                    showHyperlinkPopup: isEmbeddableElement(draggingElement) && !draggingElement.link
                        ? "editor"
    ...
    maybeDragNewGenericElement = (pointerDownState, event) => {
        ...
        if (draggingElement.type === "selection" &&
            ...
            const { snapOffset, snapLines } = snapNewElement(draggingElement, this.state, event, {
    ...
    maybeHandleResize = (pointerDownState, event) => {
        ...
        const pointerCoords = pointerDownState.lastCoords;
        let [resizeX, resizeY] = getGridPoint(pointerCoords.x - pointerDownState.resize.offset.x, pointerCoords.y - pointerDownState.resize.offset.y, event[KEYS.CTRL_OR_CMD] ? null : this.state.gridSize);
        ...
        if (!this.state.selectedElementsAreBeingDragged) {
            ...
            this.maybeCacheReferenceSnapPoints(event, selectedElements);
            const { snapOffset, snapLines } = snapResizingElements(selectedElements, getSelectedElements(originalElements, this.state), this.state, event, dragOffset, transformHandleType);
